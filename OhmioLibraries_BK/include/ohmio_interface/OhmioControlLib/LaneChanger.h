...
namespace OhmioControl
{
	...
	public:
		...
		//! @param[in] trackSpeed - speed (m/Sec) we want to drive at the transition
		//! This effects the length of the transition between lanes.
		//! THe speed for the transition track will be the maximum between current speed and track speed
		//! (or transition speed when returning) 
		//! @return true if can initiate the change
		bool Overtake(double distanceToNextLane, double currentSpeed, double trackSpeed);
