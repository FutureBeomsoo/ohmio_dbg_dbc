...
namespace OhmioTcp
{
	...
	public:
		...
		//! Get the current status of the vehicle so that it can be sent to client. public for CAN feeder
		void GetCurrentStatus(std::map<std::string, RealTimeDataMessageDefinitions::PropertyMap>& data) const;

		//! Get the current errors so that it can be sent to client. public for CAN feeder
	...
	private:
		...
		void HandleGetStatus(const std::vector<std::string>& arguments, std::string& answer);
		...
		void GetJoystickStatus(RealTimeDataMessageDefinitions::PropertyMap& status) const;
		//!@}
	...
	private:
		...
		std::shared_ptr<OhmioModel::DriveSystemStatus> m_driveStatus;			//!< Data source for motor, break, etc
		...
		std::shared_ptr<OhmioModel::JoystickStatus> m_joystick;			//!< Data source for joystick
		...
		int m_debugCount;												//!< for debugging
	};
