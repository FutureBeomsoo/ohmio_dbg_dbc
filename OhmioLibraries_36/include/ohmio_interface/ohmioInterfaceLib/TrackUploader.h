...
namespace Ohmio
{
	...
	public:
		...
		//! How long can the track be for a specific path?
		//
		//! @param[in] destination - which track is it (1/2, F/R) from enum  
		//! @return max length
		uint32_t GetMaxTrackLength(int destination);
	...
	private:
		...
		Ohmio::CAN_MSG m_in_canMsg;         //!<received can bus msg 
		Ohmio::CAN_MSG m_out_canMsg;        //!<can bus msg to be sent
