 ...
 *  @date     1 May 2020
 **/
#define ALLOCATE_LKF(LKF,N_X,N_Z) \
   ...
   const int LKF##_n_x = N_X;  \
   double LKF##_temp2[N_X * N_X]; \
   double LKF##_aux_buf_inv[SAFE_INVERSE_BUFFER_SIZE(N_Z)]; \
   double LKF##_inverted[N_Z * N_Z]; \
   double LKF##_tranposed_observation_model[N_X * N_Z]; \
   double LKF##_gain[N_X * N_Z]; \
   double LKF##_gain_t[N_Z * N_X]; \
   double LKF##_temp1[N_X * N_Z];
...
#define ASSIGN_LKF(LKF) \
   LKF.n_x = LKF##_n_x; \
   LKF.n_x_bytes = LKF.n_x * sizeof(double); \
   LKF.n_cov_bytes = LKF.n_x * LKF.n_x * sizeof(double); \
   LKF._aux_buf_inv = LKF##_aux_buf_inv; \
   LKF._inverted = LKF##_inverted; \
   LKF._tranposed_observation_model = LKF##_tranposed_observation_model; \
   LKF._gain = LKF##_gain; \
   LKF._gain_t = LKF##_gain_t; \
   LKF._temp1 = LKF##_temp1; \
   LKF._temp2 = LKF##_temp2; \
