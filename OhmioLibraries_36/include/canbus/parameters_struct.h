...
typedef struct 
{
    ...
    uint8_t minSpeedDecStepDrv;                 //'' 30 minimum deceleratoin speed in auto drive; resolution 10mm/s2 (range: min=1 max=200)
    ...
    float curveImprovement;                    //'' 50 curve improvement; the higher the value the weaker the curve turn; rang min: 10  max 1000
    uint8_t rightHandDrive;                     //'' any value other than 0 are indicated as right hand drive
    uint16_t requestAcceptStopDist;              //+  500 for general vehicle; 1500 for Lotte#5
    ...
    float steerDelta;                            //+  steering delta value
...
typedef struct
{
    ...
    float speed_kp;                           //'' kp factor for speed PID control  
    float speed_ki;                           //'' ki factor for speed PID control
    ...
    uint16_t speedIncreaseStepAuto;             //'' 5 speed rampup in mm/s after speed has been reduced during driving due to unexpected steering action range: min 1 max 1000
