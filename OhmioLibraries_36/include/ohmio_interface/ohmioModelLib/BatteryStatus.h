...
namespace OhmioModel
{
	...
	public:
		MainBatteryState() : heatRelay(false), errorLevel(0), status(0),
			...
			temperature(0), heatCurrent(0), voltage(0), charging(false), chargingCurrent(0),
			canChangeCharging(false)
		{}
		...
		int chargingCurrent;			//!< from enum BMS_CHARGE_CURRENT
		bool canChangeCharging;			//!< true if chargingCurrent can be changed via CAN message
	};
	...
	private:
		...
		void RecieveBmsChargingCurrent(const Ohmio::CAN_MSG& message);
		///@}
