...
#define ANGLE_0_360(v) (v >= 0 ? fmod(v,360) : fmod(v,360)+360)

#define ANGLE_0_2PI(v) (v >= 0 ? fmod(v,2*M_PI) : fmod(v,2*M_PI)+2*M_PI)

#define ROUND(x) ((int64_t) ((x) + (((x) > 0) - ((x) < 0)) * 0.5))

#define ROUNDED_INT_DIVISION(n, d) ((((n) < 0) == ((d) < 0)) ? (((n) + (d)/2)/(d)) : (((n) - (d)/2)/(d)))
...
// A > B with overflow for uint32
#define A_GT_B_U32(A, B) ((((uint32_t)B)-((uint32_t)A)) > ((uint32_t)0x80000000))
// A >= B with overflow for uint32
#define A_GE_B_U32(A, B) ((((uint32_t)B)-((uint32_t)A)) == 0 || (((uint32_t)B)-((uint32_t)A)) > ((uint32_t)0x80000000))


#ifdef __XC32__
/**
 * @brief memcpy wrapper to save the callers address (usefull for debugging PIC32)
 */
void * memcpy_dbg(void * d, const void *s , size_t n);
#endif
