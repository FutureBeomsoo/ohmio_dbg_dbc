...
namespace OhmioModel
{
	...
	//! Waht is the air condition doing
	class AcStatus
	{
	public:
		AcStatus() 
			...
			, errorCode(0)
			, errorLevel(0)
			, numLevels(3) // default (old air con) 3 levels
			, mode(AirConMode::lastMode)
			, requestedTemperature(-500) // deliberatly unreasonable
			, airConTemp1(0)
			, airConTemp2(0)
			, roomTemp(0)
			, outdoorTemp(0)
		...
		int manufecturareCode;
		int versionMajor;
		int versionMinor;
		int fanLevel;
		int errorCode;
		int errorLevel;
        int numLevels;		//!< how many fan levels we have
		AirConMode mode;
	...
	public:
		...
		bool reverse;			//!< reverse beep should be on
	...
	public: 
		...
		//
		//! @param[out] status - all the information we have
		void GetAirConState(AcStatus& status) const;
	...
	private:
		...
		void UpdateAvsRequest(const Ohmio::CAN_MSG & message);
		void UpdateHornState(const Ohmio::CAN_MSG & message);
		void UpdateAirConState(const Ohmio::CAN_MSG & message);
		void UpdateAirconStateK165(const Ohmio::CAN_MSG & message);
		void UpdateAirconVersion(const Ohmio::CAN_MSG & message);
		///@}
	...
	private:
		...
		AcStatus m_airCon;		//!< what is the AC doing?
		bool m_hornOn;			//!< is the horn on
