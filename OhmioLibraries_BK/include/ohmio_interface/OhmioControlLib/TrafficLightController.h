#pragma once
#include "ohmioInterfaceLib/V2Xhandler.h"
...
namespace OhmioControl
{
	...
	class TrafficLightController : public Ohmio::IV2XConsumer , public Threading::RunInThreadForever
	{
	public:
		...
		//! For emulator turn off error when we have wrong intersection
		void DisableErrorIfWrongSignal();

		//! Set driving to stop line if V2X error
		void AlwaysDriveToStopLine(bool drive);

		// IV2XConsumer interface
		void Consume(std::string message, bool isLive) override;
	...
	private:
		...
		bool m_disableWrongLightError;	//!< for emulator testing don't generate error if the wrong intersection data
		...
		std::shared_ptr<TrafficLightDecisionTree> m_decisionTree;	//!< to decide what to do
