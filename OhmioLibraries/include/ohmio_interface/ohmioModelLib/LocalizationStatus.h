...
namespace OhmioModel
{
	...
	{
	public:
		ImuData() : Pitch(0), Roll(0), Heading(0), AccelerationX(0), 
			AccelerationY(0), AccelerationZ(0), GyroPitch(0), GyroRoll(0), 
			GyroHeading(0), Temperature(0), status(0) {};

        double Pitch;			//!< pitch angle (degrees)	
        double Roll;			//!< roll angle (degrees)
        double Heading;			//!< heading angle (degrees)
        double AccelerationX;	//!< acceleration in X direction (m/sec^2)
		double AccelerationY;	//!< acceleration in Y direction (m/sec^2)
		double AccelerationZ;	//!< acceleration in Z direction (m/sec^2)
        double GyroPitch;		//!< Pitch rate (degrees/sec)
		double GyroRoll;		//!< Roll rate (degrees/sec)
		double GyroHeading;		//!< Heading rate (degrees/sec)
	...
	public:
		FusionData() : Heading(0), ForwardConfidence(0), LateralConfidence(0), 
			...
			AntennaDist(0), AntennaDistExpected(0), Pitch(0), Roll(0), Height(0), Speed(0),
			WayPointIndex(0), GpsDisabled(false), GpsFrontInUse(false), GpsRearInUse(false), LidarInIse(false), 
			CameraInIse(false), Initializable(false), AntenaOutOfRange(false)
		{}
		...
		bool Initializable;			//!< GPS data is good and can be use for location
		bool AntenaOutOfRange;		//!< GPS bad because distance out of range
	};
	...
	public:
		...
		//! Get the current status of the IMU
		virtual void GetImuData(ImuData& data) = 0;
	...
	public:
		...
		//! Get the current status of the IMU
		void GetImuData(ImuData& data) override;
	...
	private:
		...
		void RecieveFusionExtraFlagsMessage(const Ohmio::CAN_MSG& message);
		void RecieveCompasMessage(const Ohmio::CAN_MSG&message);
		void RecieveImuRotationMessage(const Ohmio::CAN_MSG&message);
		void RecieveImuAccelerationMessage(const Ohmio::CAN_MSG&message);
		void RecieveImuGyroMessage(const Ohmio::CAN_MSG&message);
		void RecieveImuStatusMessage(const Ohmio::CAN_MSG&message);
		///@}
