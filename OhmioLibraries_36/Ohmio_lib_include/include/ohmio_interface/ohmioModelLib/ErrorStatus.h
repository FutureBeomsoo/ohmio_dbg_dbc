...
namespace OhmioModel
{
	...
	public:
		...
		{
		public:
			int unitID;				//!< which unit
			int taskID;				//!< which task
			uint32_t data;			//!< data
		};
		...
		void GetReasetReasons(errorMap& status);
		//!@}
		...
		void GetUnitDebugMessage(int unitId, std::vector<debugMessage>& messages);

		//! Get debug message for all units
		void GetDebugMessages(std::vector<debugMessage>& messages);
	...
	private:
		...
		std::vector<debugMessage> m_debugMessage;		//!< debug message for each unit
