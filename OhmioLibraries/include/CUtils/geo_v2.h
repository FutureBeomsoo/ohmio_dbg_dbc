...
/**
 *  @brief     Rotates a lat/lon covariance into a east/north covariance. It uses the Jacobian (J * M * J') of the geo2enu transform in this file.
 *  @author    Carlos Aydos
 *  @date      06 Jan 2024
 **/
void geo2enu_2D_covariance(
      double const * const m, //!< INPUT - 2x2 matrix (in radians)
      double lat0,            //!< INPUT - reference latitude (in radians)
      double * const out      //!< OUTPUT - 2x2 rotated covariance (in metres)
      );

/**
 *  @brief     Same as geo2enu_2D_covariance, but using micro radians
 *  @author    Carlos Aydos
 *  @date      06 Jan 2024
 **/
void geo2enu_2D_covariance_micro(
      double const * const m, //!< INPUT - 2x2 matrix (in micro radians)
      double lat0,            //!< INPUT - reference latitude (in micro radians)
      double * const out      //!< OUTPUT - 2x2 rotated covariance (in metres)
      );

/**
 *  @brief     Rotates a east/north covariance into a lat/lon covariance. It uses the Jacobian (J * M * J') of the enu2geo transform in this file.
 *  @author    Carlos Aydos
 *  @date      09 Jan 2024
 **/
void enu2geo_2D_covariance(
      double const * const m, //!< INPUT - 2x2 matrix (in metres)
      double lat0,            //!< INPUT - reference latitude (in radians)
      double * const out      //!< OUTPUT - 2x2 rotated covariance (in radians)
      );

/**
 *  @brief     Same as enu2geo_2D_covariance, but using micro radians
 *  @author    Carlos Aydos
 *  @date      09 Jan 2024
 **/
void enu2geo_2D_covariance_micro(
      double const * const m, //!< INPUT - 2x2 matrix (in metres)
      double lat0,            //!< INPUT - reference latitude (in micro radians)
      double * const out      //!< OUTPUT - 2x2 rotated covariance (in micro radians)
      );
...
/**
 * @brief As enu2geo, but ref latitude, longitude and rotation expressed in degrees
 * and the return is also in degrees
 */
void enu2geo_degrees(
	const pos_cartesian_t * const enu,
	const pos_geodetic_t * const geo0,
	double rotation,
	pos_geodetic_t * const geo_pos
);
