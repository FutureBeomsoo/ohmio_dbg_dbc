...
namespace OhmioTcp
{
	...
	public:
		...
		//! @param[out] moduleName - the module to pass the command to
		//! @param[out] command - the command to perform
		//! @param[out] arguments - list of parameters
		//! @return true if this can be parsed
		static bool ParseMessage(const std::string& message, std::string& moduleName,
			int& command, std::vector<std::string>& arguments);


	private:
		//! Find (or create) a module by name
		std::shared_ptr<OhmioTcp::IModuleServer> FindModule(const std::string& moduleName);

	private:
		std::map<std::string, std::shared_ptr<IModuleServer>> m_modules;	//!< all the modules that we need
