...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			uint64_t right_count :32;
			uint64_t left_count :32;
		} signals;
	} raw;
} WheelCountMsg_t;
...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			...
			uint64_t fus_conflicting_inputs :1;
...
static void FusionLatitudeMsg_ToCan(FusionLatitudeMsg_t * msg) 
{ 
    assert(msg->latitude >= -90 && msg->latitude <= 90);
...
static void FusionLatitudeMsg_FromCan(FusionLatitudeMsg_t * msg) 
{ 
    if (msg->raw.signals.sign)
       msg->latitude = -(msg->raw.signals.latitude * 0.00000001);
    else
...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			...
			uint64_t w_huge_sensor_update :1;
			...
			uint64_t st_front_gnss_no_data_can :1;
			uint64_t st_rear_gnss_no_data_can :1;
			...
			uint64_t st_recent_gnss_available :1;
...
enum FusionDebugMsgErrorCode
{
	...
	fusdebug_odometry_Null,
	fusdebug_rtk_Front_Null,
	fusdebug_rtk_Rear_Null,
...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			...
			uint64_t odometryNull :1;
			uint64_t rtkFrontNull :1;
			uint64_t rtkRearNull :1;
...
// message ID 0x7F0 PrintASCIIMsg
typedef struct{ 
	union
	{ 
		uint8_t buf[8];
...
// message ID 0x26 ErrorTrans
enum TransErrorCode
{
	Trans_Joystick_Disconnect = 0,
	Trans_Joystick_Comms,
	Trans_Joystick_Centre,
	Trans_Safety_Relay_Open,
	//always add error codes above this
	Trans_NumErrorCodes,
};
...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			uint64_t TransJoystickDisconnect :1;
			uint64_t TransJoystickCommsError :1;
			uint64_t TransJoystickCentreError :1;
			uint64_t TransSafetyRelayOpen :1;
		} signals;
	} raw;
} ErrorTrans_t;
...
typedef struct{ 
	...
	{ 
		...
		struct
		{
			uint64_t AtlocGPSInputSearchError :1;
			uint64_t AtlocUWBInputSearchError :1;
...
// message ID 0x2D ErrorMotorAdaptor
typedef struct{ 
	union
	{ 
		uint8_t buf[8];
