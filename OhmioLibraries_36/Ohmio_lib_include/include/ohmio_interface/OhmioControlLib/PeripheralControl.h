...
namespace OhmioControl
{
	...
	public:
		...
		//! Call with false to set the horn to be controlled by joystick commands
        //! @param[in] on - true to turn the horn on, false to turn it off
		void BeepHorn(bool on);

		//! Get the air condition information
		//
		//! @param[out] status - all the information we have
		void GetAirConState(OhmioModel::AcStatus& status);
		...
		void GetAirConTempRange(double& minTemp, double& maxTemp, int& numFanLevels);
